          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <button
              ...
              className={`mt-2 w-full rounded-2xl px-4 py-3 font-semibold text-white transition shadow
                ${
                  !allValid || submitting
                    ? "bg-[#8A6B52]/40 cursor-not-allowed"
                    : "bg-[#F07818] hover:brightness-110 cursor-pointer"
                }`}
            >
...
function Field({
  ...
  return (
    <div>
      ...
      {hint && <p className="mt-1 text-xs text-[#8A6B52]/70">{hint}</p>}
    </div>
  );
}

function GenderChip({ label, value, current, onChange }) {
  ...
  return (
    <button
      ...
      className={`flex-1 rounded-xl border px-3 py-2 text-sm
        ${
          selected
            ? "border-[#F07818] bg-[#FFF1E5] text-[#F07818]"
            : "border-[#E6D9CC] text-[#8A6B52] bg-white"
        }`}
    >
      {label}
    </button>
